            ...
            <div class="col-12">
                <div class="form-group mb-3 @error('address.name') has-error @enderror">
                    <input type="text" name="address[name]" id="address_name" placeholder="{{ __('Full Name') }}"
                        ...
                        value="{{ old('address.name', Arr::get($sessionCheckoutData, 'name')) }}" required>
            ...
            <div class="col-lg-8 col-12">
                <div class="form-group  @error('address.email') has-error @enderror">
                    <input type="email" name="address[email]" id="address_email" placeholder="{{ __('Email') }}"
                        class="form-control address-control-item address-control-item-required checkout-input"
                        value="{{ old('address.email', Arr::get($sessionCheckoutData, 'email')) }}" required>
                    {!! Form::error('address.email', $errors) !!}
                </div>
            </div>
            <div class="col-lg-4 col-12">
                <div class="form-group  @error('address.phone') has-error @enderror">
                    {!! Form::phoneNumber('address[phone]', old('address.phone', Arr::get($sessionCheckoutData, 'phone')), ['id' => 'address_phone', 'required' => 'required','class' => 'form-control address-control-item ' . (!EcommerceHelper::isPhoneFieldOptionalAtCheckout() ? 'address-control-item-required' : '') . ' checkout-input']) !!}
        ...
        <div class="row">
                        ...
                        <div class="select--arrow">
                            <select name="address[country]" class="form-control address-control-item address-control-item-required" required
                                ...
                                @foreach(EcommerceHelper::getAvailableCountries() as $countryCode => $countryName)
                                    <option value="{{ $countryCode }}" @if (old('address.country', Arr::get($sessionCheckoutData, 'country')) == $countryCode) @endif @if (35 == $countryCode)  selected @endif>{{ $countryName }}</option>
                                @endforeach
                            </select>
                            <i class="fas fa-angle-down"></i>
                        </div>
                    ...
                    @if (EcommerceHelper::loadCountriesStatesCitiesFromPluginLocation())
                        <div class="select--arrow">
                            <select name="address[state]" class="form-control address-control-item address-control-item-required" required
                                ...
                                @if (old('address.country', Arr::get($sessionCheckoutData, 'country')) || !EcommerceHelper::isUsingInMultipleCountries() || 35)
            ...
            <div class="col-sm-6 col-12">
                <div class="form-group  @error('address.city') has-error @enderror">
                    @if (EcommerceHelper::loadCountriesStatesCitiesFromPluginLocation())
                        <div class="select--arrow">
                            <select name="address[city]" class="form-control address-control-item address-control-item-required" id="address_city" required data-type="city" data-url="{{ route('ajax.cities-by-state') }}">
            ...
            <div class="col-12">
                <div class="form-group mb-3 @error('address.address') has-error @enderror">
                    <input id="address_address" type="text" class="form-control address-control-item address-control-item-required checkout-input" placeholder="{{ __('Address') }}" name="address[address]" value="{{ old('address.address', Arr::get($sessionCheckoutData, 'address')) }}" required>
                ...
                <div class="col-12">
                    <div class="form-group mb-3 @error('address.zip_code') has-error @enderror">
                        <input id="address_zip_code" type="text"
                            ...
                            value="{{ old('address.zip_code', Arr::get($sessionCheckoutData, 'zip_code')) }}"  readonly onfocus="this.removeAttribute('readonly');" required>
